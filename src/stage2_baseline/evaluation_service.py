...
class EvaluationService:
    ...
    @staticmethod
    def load_suite_report(file_path: str) -> ModelEvaluationSuiteReport:
        """Load a saved evaluation suite report JSON file into a ModelEvaluationSuiteReport entity."""
        from src.infrastructure.persistence import load_json
        data = load_json(file_path)
        report = ModelEvaluationSuiteReport(
            model_name=data.get("model_name", "Unknown"),
            checkpoint_path=data.get("checkpoint_path"),
            timestamp=data.get("timestamp"),
            ladder_auc=data.get("ladder_auc", 0.0),
            collapse_point=data.get("collapse_point", "N/A"),
            consistency_delta=data.get("consistency_delta", 0.0),
        )
        for k, v in data.get("level_reports", {}).items():
            report.level_reports[k] = LevelEvaluationReport(
                ladder_level=v.get("level", k),
                benchmark_name=v.get("benchmark", ""),
                total_tasks=v.get("total_tasks", 0),
                pass_at_1=v.get("pass_at_1", 0.0),
                pass_at_5=v.get("pass_at_5"),
                error_breakdown=v.get("error_breakdown", {}),
                task_results=v.get("task_results", []),
            )
        return report
